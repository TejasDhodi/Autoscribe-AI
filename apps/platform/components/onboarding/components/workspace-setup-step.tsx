...
export function WorkspaceSetupStep({
  ...
  function validateField(name: string, value: any) {
    switch (name) {
      ...
      case "teamName":
        if (!value.trim())
          return `${name === "workspaceName" ? "Workspace" : "Team"} name is required.`;
      ...
      // if (!validImagesTypes.includes(value.type)) return "Invalid file type.";
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between">
        ...
        <AnimatedButton onClick={handleNext} disabled={false} className="gap-2">
          Continue
        </AnimatedButton>
      </div>
    </motion.div>
